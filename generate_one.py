...
generators = {
    ...
    "s_sea_r1": "generators.SEAGenerator -f 1 -i 3",
    "s_sea_r2": "generators.SEAGenerator -f 2 -i 2",
    "s_sea_r3": "generators.SEAGenerator -f 3 -i 1"
}
drift_pairs = [
    ...
    ["s_sea_r1","s_sea_r2"], ["s_sea_r2","s_sea_r3"], ["s_sea_r1","s_sea_r3"]
]
...
for drift_pair in drift_pairs:
    ...
    streams = [g_first, g_second]
    filename = "osd_%s_%s" % (first, second)
    print(filename)
...
for drift_pair in drift_pairs:
    ...
    g_first, g_second = generators[first], generators[second]
    streams = [g_first, g_second]
    filename = "oid_%s_%s" % (first, second)
    print(filename)
    h.generate_stream(path % filename, length, window, streams)
    # test(filename)
